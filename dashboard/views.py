...
def dashboard_monitor(request):
    """Camera monitoring dashboard page.

    This first version uses temporary in-view data so the dashboard page can be built
    before the real monitor models, ESP32-CAM upload endpoint, and alert storage are added.
    """
    ...
    camera_modules = [
        {
            ...
            'module_variant': 'AI-Thinker ESP32-CAM',
            'status': 'online',
            'status_label': 'Online',
            'last_seen': 'Just now',
            'last_upload': '3 mins ago',
            'battery_level': 92,
            'wifi_rssi': -54,
        ...
        },
        {
            ...
            'last_upload': '25 mins ago',
            'battery_level': 37,
            'wifi_rssi': -72,
            'location': 'Trail checkpoint B',
        },
    ]
    ...
    monitor_alerts = [
        {
            ...
            'detected_activity': 'Protected plant disturbance',
            'confidence': '84%',
            'camera_id': 'CAM-001',
            'camera_product_name': 'ESP32-CAM',
            'guide_name': 'test user',
            'location': 'Field monitoring preview',
            'captured_at': '2026-05-03 15:20',
            'received_at': '2026-05-03 15:23',
            'video_filename': 'protected-plant-disturbance-preview.mp4',
            'video_duration': '00:18',
            'evidence_status': 'Placeholder video not attached',
            'recommended_action': 'Review the returned footage and confirm whether the detected activity violates park regulations.',
            'details': 'This placeholder alert represents the future AI detection workflow. When the backend is connected, this dashboard card will display the actual uploaded video clip, detection result, confidence score, guide/session details, and review status.',
        },
        {
            'id': 'ALERT-002',
            ...
            'severity': 'Medium',
            'status': 'Pending review',
            'detected_activity': 'Person near restricted object',
            'confidence': '91%',
            'camera_id': 'CAM-001',
            'camera_product_name': 'ESP32-CAM',
            'guide_name': 'test user',
            'location': 'Field monitoring preview',
            'captured_at': '2026-05-03 15:10',
            'received_at': '2026-05-03 15:13',
            'video_filename': 'person-near-restricted-object-preview.mp4',
            'video_duration': '00:21',
            'evidence_status': 'Placeholder video not attached',
            'recommended_action': 'Check the footage and verify whether the detected activity requires intervention or documentation.',
            'details': 'This placeholder alert is used to test the dashboard evidence-review flow before the real camera module and backend alert storage are completed.',
        },
    ]

    if selected_status:
        camera_modules = [item for item in camera_modules if item['status'] == selected_status]

    if selected_severity:
        monitor_alerts = [item for item in monitor_alerts if item['severity'].lower() == selected_severity.lower()]

    if search_query:
        normalized_query = search_query.lower()

        camera_modules = [
            item for item in camera_modules
            if normalized_query in item['id'].lower()
            or normalized_query in item['product_name'].lower()
            or normalized_query in item['module_variant'].lower()
            or normalized_query in item['location'].lower()
        ]

        monitor_alerts = [
            item for item in monitor_alerts
            if normalized_query in item['title'].lower()
            or normalized_query in item['summary'].lower()
            or normalized_query in item['camera_id'].lower()
            or normalized_query in item['guide_name'].lower()
            or normalized_query in item['location'].lower()
            or normalized_query in item['detected_activity'].lower()
        ]

    stats = {
        'total_cameras': len(camera_modules),
        'online_cameras': len([item for item in camera_modules if item['status'] == 'online']),
        'issue_cameras': len([item for item in camera_modules if item['status'] == 'issue']),
        'total_alerts': len(monitor_alerts),
        'pending_alerts': len([item for item in monitor_alerts if item['status'] == 'Pending review']),
        'high_severity_alerts': len([item for item in monitor_alerts if item['severity'] == 'High']),
    }
