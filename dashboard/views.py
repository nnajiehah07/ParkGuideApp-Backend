...
def dashboard_monitoring(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'upload_test_video':
            ...
            camera_source = request.POST.get('camera_source', '').strip() or 'RE-CAM-01'
...
def get_monitoring_dashboard_summary():
    total_alerts = ViolationAlert.objects.count()
...
# Deprecated: Old sample-data view replaced by real dashboard_monitoring view with DB-backed data.
# See dashboard_monitoring() for the current monitoring page.
